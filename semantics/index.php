...
<body>
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $sql2 = "SELECT id, title, content_version, version_title FROM woh_content WHERE id=\"$id\" AND content_version=$v AND content_language=\"$lang\"";
        $result2 = $mysqli->query($sql2);
        while ($row2 = $result2->fetch_assoc()) {
            $sql_chapter = "SELECT tag FROM woh_tags WHERE (id = '" . $row2["id"] . "' AND tag = 'chapter')";
            ...
            if ($num_chap == 0) {
                ...
                $sql_title = "SELECT title FROM woh_content JOIN woh_web ON woh_web.parent_id = woh_content.id WHERE woh_web.child_id = '" . $row2["id"] . "' AND woh_web.child_version = " . $row2["content_version"];
