...
function populateHead($id)
{
    ...
    $sql = "SELECT title, snippet, large_image FROM MnL_metadata WHERE MnL_metadata.id = \"" . $id . "\"";
    // IFNULL(large_image, (SELECT large_image FROM MnL_web JOIN MnL_metadata ON MnL_web.parent_id = MnL_metadata.id WHERE MnL_web.child_id = \"" . $id . "\" LIMIT 1))
    ...
    if ((!($id == 0)) && ($num_rows == 0)) {
        echo "<meta http-equiv=\"Refresh\" content=\"0; url='https://MnLandlegacy.com/404/'\" />";
    }
    while ($row = $result->fetch_assoc()) {
        if (is_null($row["large_image"])) {
            echo "<meta content='" . strip_tags($row["title"]) . " | Myths and Legacy' property='og:title'/>
                    <meta content='" . $row["snippet"] . " | Myths and Legacy' property='og:description'/>
                    <meta content='http://www.MythsandLegacy.co/img/ogp.png' property='og:image'/>
                    <meta content='summary_large_image' name='twitter:card'/>
                    <meta content='@MythsLegacy' name='twitter:site'/>
                    <title>" . strip_tags($row["title"]) . " | Myths and Legacy</title>";
        } else {
            echo "<meta content='" . strip_tags($row["title"]) . " | Myths and Legacy' property='og:title'/>
                    <meta content='" . $row["snippet"] . " | Myths and Legacy' property='og:description'/>
                    <meta content='http://www.MythsandLegacy.co/img/ogp.png' property='og:image'/>
...
function hasChildren($id)
{
    ...
    $sql = "SELECT child_id AS cid, title, snippet, small_image, large_image, chronology FROM MnL_web JOIN MnL_metadata ON MnL_web.child_id = MnL_metadata.id WHERE MnL_web.parent_id = \"" . $id . "\" ORDER BY IFNULL(chronology, (SELECT chronology FROM MnL_web JOIN MnL_metadata ON MnL_web.child_id = MnL_metadata.id WHERE MnL_web.parent_id = cid ORDER BY chronology LIMIT 1)), title ASC";
    ...
    if ($id == "0") {
        $sql = "SELECT id AS cid, title, snippet, small_image, large_image, chronology FROM MnL_metadata WHERE id NOT IN (SELECT child_id FROM MnL_web) ORDER BY IFNULL(chronology, (SELECT MIN(chronology) FROM MnL_web JOIN MnL_metadata ON MnL_web.child_id = MnL_metadata.id WHERE MnL_web.parent_id = cid AND chronology IS NOT NULL)), title ASC";
    ...
    if ($num_rows != 0) {
        ...
        if ($id != "0") {
            $sql_standalone = "SELECT child_id FROM MnL_web JOIN MnL_tags ON MnL_web.child_id=MnL_tags.id WHERE MnL_web.parent_id = '$id' AND MnL_tags.tag ='chapter'";
...
{
    include("..//php/db_connect.php");
    $sql_header = "SELECT html FROM MnL_content JOIN MnL_headers ON MnL_content.header = MnL_headers.header_id WHERE MnL_content.id = '$id' LIMIT 1";
    // Make this recurse up to get parents if none.
    $result_header = $mysqli->query($sql_header);
    $num_rows = mysqli_num_rows($result_header);

    if ($num_rows == 0) {
        echo '<img src="/img/MYTHSLEGACYlogo.png" alt="Myths and Legacy" height="80" width="104" style="cursor: pointer;" onclick="window.location.href=\'/\'">';
    } else {
...
function loadContent($id)
{
    ...
    $sql = "SELECT * FROM MnL_web WHERE child_id = \"" . $id . "\"";
    ...
    if ((!($id == 0)) && ($num_rows == 0)) {
        ...
        while ($row = $result->fetch_assoc()) {
            if ($num_rows == 1) {
                $sql_title = "SELECT title FROM MnL_metadata WHERE id = \"" . $row["parent_id"] . "\"";
    ...
    $sql = "SELECT title FROM MnL_metadata WHERE id = \"" . $id . "\"";
    ...
    $sql = "SELECT tag FROM MnL_tags WHERE id = \"" . $id . "\" AND (tag_type = 'developer' OR tag_type = 'author' OR tag_type = 'illustrator')";
    ...
    $sql = "SELECT main FROM MnL_content WHERE id = \"" . $id . "\"";
...
function populateSettings()
{
    ...
    $sql_tags = "SELECT DISTINCT(tag) AS tag, detailed_tag FROM MnL_tags WHERE (tag_type = 'type' OR tag_type = 'language' OR tag_type = 'organizational' OR tag_type = 'author') ORDER BY tag_type";
    ...
    $sql = "SELECT child_id AS cid, title, chronology FROM MnL_web JOIN MnL_metadata ON MnL_web.child_id = MnL_metadata.id WHERE child_id NOT IN (SELECT parent_id FROM MnL_web) ORDER BY IFNULL(chronology, (SELECT chronology FROM MnL_web JOIN MnL_metadata ON MnL_web.child_id = MnL_metadata.id WHERE MnL_web.parent_id = cid ORDER BY chronology LIMIT 1)) ASC, title ASC";
    $result = $mysqli->query($sql);
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $sql_chapter = "SELECT tag FROM MnL_tags WHERE (id = '" . $row["cid"] . "' AND tag = 'chapter')";

        $result_chapter = $mysqli->query($sql_chapter);
        $num_chap = mysqli_num_rows($result_chapter);
        if ($num_chap == 0) {
            $sql_nexttags = "SELECT GROUP_CONCAT(tag SEPARATOR ', ') AS tags FROM MnL_tags WHERE (tag_type = 'type' OR tag_type = 'language' OR tag_type = 'organizational' OR tag_type = 'author') AND id = '" . $row["cid"] . "'";
            ...
            $sql_title = "SELECT title FROM MnL_metadata JOIN MnL_web ON MnL_web.parent_id = MnL_metadata.id WHERE MnL_web.child_id = '" . $row["cid"] . "'";
